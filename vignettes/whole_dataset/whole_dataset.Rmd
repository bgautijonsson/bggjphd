...
theta_files <- glue::glue("theta_samp-{c(1, 3, 4)}.csv")
...
x_files <- glue::glue("x_samp-{c(1, 3, 4)}.csv")
...
make_posterior_data <- function(file) {
  file |> 
    vroom::vroom(skip = 1) |> 
    mutate(iter = row_number()) |> 
    pivot_longer(c(-iter), names_to = "variable", values_to = "value") |> 
    mutate(
      variable = rep(c("psi", "tau", "phi", "gamma"), each = nrow(stations)) |> as_factor(),
      station = rep(seq_len(nrow(stations)), 4),
      .by = iter
    ) |> 
...
d |> 
  write_parquet(
    "data/posterior_samples.parquet"
  )
```
